...
export default function ColumnWithBoxes({boxes, groupby}) {
    ...
    return(
        <section>
            ...
            {boxes.map((box) => (
              <Box key={box.id} id={box.id} user={box.username} timeupload={box.submit_time} tag={box[tagKey]} address={box.address} description={box.description}/>
            ))}
            </div>
        </section>
    )
